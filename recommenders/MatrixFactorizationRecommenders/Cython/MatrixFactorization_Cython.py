...
class _MatrixFactorization_Cython(BaseMatrixFactorizationRecommender, Incremental_Training_Early_Stopping):
    ...
    def fit(self, URM_train, verbose=True, recompile_cython = False, algorithm_name = "MF_BPR", epochs=300, batch_size=1000,
        ...
        assert negative_interactions_quota >= 0.0 and negative_interactions_quota < 1.0,\
            "{}: negative_interactions_quota must be a float value >=0 and < 1.0, provided was '{}'".format(self.RECOMMENDER_NAME, negative_interactions_quota)
    ...
    def runCompilationScript(self):
        ...
        file_subfolder = "/recommenders/MatrixFactorizationRecommenders/Cython"
...
class MatrixFactorization_BPR_Cython(_MatrixFactorization_Cython):
    ...
    def fit(self, URM_train, verbose=True, *pos_args, algorithm_name="MF_BPR", **key_args):

        super(MatrixFactorization_BPR_Cython, self).instanziate_rec_cython(URM_train, verbose, *pos_args, algorithm_name="MF_BPR", **key_args)
        key_args["use_bias"] = False
        key_args["negative_interactions_quota"] = 0.0

        super(MatrixFactorization_BPR_Cython, self).fit(URM_train, verbose, **key_args)
...
class MatrixFactorization_FunkSVD_Cython(_MatrixFactorization_Cython):
    ...
    def fit(self, URM_train, verbose = True, *pos_args, algorithm_name="FUNK_SVD", **key_args):
        super(MatrixFactorization_FunkSVD_Cython, self).instanziate_rec_cython(URM_train, verbose, *pos_args, algorithm_name="FUNK_SVD", **key_args)
...
class MatrixFactorization_AsySVD_Cython(_MatrixFactorization_Cython):
    ...
    def fit(self,URM_train, verbose =  True, *pos_args, algorithm_name="ASY_SVD", **key_args):
        super(MatrixFactorization_AsySVD_Cython, self).instanziate_rec_cython(URM_train, verbose, *pos_args, algorithm_name="ASY_SVD", **key_args)
